...
import { createAbbr, enhanceAbbrs } from '../../../../src/elements/inline/abbr';
...
describe('elements/inline/abbr', () => {
  ...
  it('creates an <abbr> via document.createElement (never createElementNS)', () => {
    const el = createAbbr();
    const fake = el as unknown as FakeElement;

    expect(calls.createElement).toEqual(['abbr']);
    expect(calls.createElementNS).toEqual([]);
    expect(fake.tagName).toBe('ABBR');
  });

  it('sets text via textContent (never HTML interpretation)', () => {
    const el = createAbbr('<b>NASA</b>');
    const fake = el as unknown as FakeElement;

    expect(fake.textContent).toBe('<b>NASA</b>');
  });

  it('forwards curated global attributes (including title for expansions)', () => {
    const el = createAbbr('NASA', {
      id: 'abbr-1',
      className: 'abbr',
    ...
    expect(fake.id).toBe('abbr-1');
    expect(fake.className).toBe('abbr');
    expect(fake.title).toBe('National Aeronautics and Space Administration');
  ...
  it('supports structured ARIA input (mapped to aria-* attributes)', () => {
    ...
    expect(fake.getAttribute('aria-label')).toBe('Abbreviation');
  ...
  it('enhanceAbbrs is a no-op (but stable hook)', () => {
    expect(() => enhanceAbbrs()).not.toThrow();
  });
});
