...
import {
  ...
  createSmall,
  enhanceSmalls,
} from '../../../../src/elements/inline/small';
...
describe('elements/inline/small', () => {
  it('exposes tag and selector constants', () => {
    expect(SMALL_TAG).toBe('small');
    expect(SMALL_SELECTOR).toBe('small');
  });

  it('creates a <small> via document.createElement', () => {
    const doc = installFakeDocument();

    const el = createSmall();

    expect(doc.createElement).toHaveBeenCalledTimes(1);
    expect(doc.createElement).toHaveBeenCalledWith('small');
  ...
  it('assigns text via textContent (not innerHTML)', () => {
    ...
    const el = createSmall('Small print.');

    expect(el.textContent).toBe('Small print.');
  ...
  it('applies global attributes', () => {
    ...
    expect(el.className).toBe('fineprint');
    expect(el.title).toBe('Disclosure');
  ...
  it('maps structured ARIA into aria-* attributes', () => {
    ...
    const el = createSmall('Hello', {
      aria: { label: 'Small print', labelledby: 'small-label', hidden: true },
    });

    expect(el.getAttribute('aria-label')).toBe('Small print');
    expect(el.getAttribute('aria-labelledby')).toBe('small-label');
  ...
  it('enhancement hook is a stable no-op', () => {
    ...
    expect(() => enhanceSmalls()).not.toThrow();
    expect(() => enhanceSmalls(document as unknown as ParentNode)).not.toThrow();
  });
});
