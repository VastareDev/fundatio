...
import { createRtc, enhanceRtcs, RTC_TAG } from '../../../../src/elements/inline/rtc';
...
describe('elements/inline/rtc', () => {
  it('creates an <rtc> using document.createElement (not createElementNS)', () => {
    const doc = installFakeDocument();

    try {
      const el = createRtc();

      expect(doc.created).toEqual([RTC_TAG]);
      expect(doc.createdNS).toEqual([]);

      expect((el as unknown as FakeElement).tagName).toBe(RTC_TAG);
    } finally {
      doc.restore();
    }
  });
  ...
  it('assigns text via textContent', () => {
    ...
    try {
      const el = createRtc('San Francisco');

      expect(doc.created).toEqual([RTC_TAG]);
      expect((el as unknown as FakeElement).textContent).toBe('San Francisco');
    } finally {
      doc.restore();
    }
  });
  ...
  it('applies global attributes (id, className, lang, dir, title, booleans)', () => {
    ...
    try {
      const el = createRtc('x', {
        id: 'rtc-1',
        className: 'ruby-meaning',
        lang: 'en',
        dir: 'ltr',
        title: 'Ruby meaning',
        hidden: true,
      ...
      expect(fe.id).toBe('rtc-1');
      expect(fe.className).toBe('ruby-meaning');
      expect(fe.lang).toBe('en');
      expect(fe.dir).toBe('ltr');
      expect(fe.title).toBe('Ruby meaning');
  ...
  it('maps structured ARIA into aria-* attributes', () => {
    ...
    try {
      const el = createRtc(undefined, {
        aria: {
          label: 'Ruby annotation container',
      ...
      const fe = el as unknown as FakeElement;
      expect(fe.getAttribute('aria-label')).toBe('Ruby annotation container');
  ...
  it('enhancement hook is stable (no-op in v1.0.0)', () => {
    ...
    try {
      expect(() => enhanceRtcs()).not.toThrow();
    } finally {
      doc.restore();
    }
  });
