...
import { I_TAG, I_SELECTOR, createI, enhanceIs } from '../../../../src/elements/inline/i';
...
describe('elements/inline/i', () => {
  it('exposes tag and selector constants', () => {
    expect(I_TAG).toBe('i');
    expect(I_SELECTOR).toBe('i');
  });

  it('creates an <i> via document.createElement', () => {
    const doc = installFakeDocument();

    const el = createI();

    expect(doc.createElement).toHaveBeenCalledTimes(1);
    expect(doc.createElement).toHaveBeenCalledWith('i');
  ...
  it('assigns text via textContent (not innerHTML)', () => {
    ...
    const el = createI('Idiomatic.');

    expect(el.textContent).toBe('Idiomatic.');
  ...
  it('applies global attributes', () => {
    ...
    const el = createI('Hello', {
      id: 'i1',
      className: 'alt-voice',
      title: 'Alternate voice',
      lang: 'en',
      dir: 'ltr',
      hidden: true,
      tabIndex: 1,
    ...
    expect(el.className).toBe('alt-voice');
    expect(el.title).toBe('Alternate voice');
  ...
  it('applies dataset and style safely', () => {
    ...
    expect((el.style as Record<string, unknown>).fontStyle).toBe('italic');
  });
  ...
  it('maps structured ARIA into aria-* attributes', () => {
    ...
    const el = createI('Hello', {
      aria: { label: 'Alternate voice', labelledby: 'i-label', hidden: true },
    });

    expect(el.getAttribute('aria-label')).toBe('Alternate voice');
    expect(el.getAttribute('aria-labelledby')).toBe('i-label');
  ...
  it('enhancement hook is a stable no-op', () => {
    ...
    expect(() => enhanceIs()).not.toThrow();
    expect(() => enhanceIs(document as unknown as ParentNode)).not.toThrow();
  });
});
