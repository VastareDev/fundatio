...
import { createRp, enhanceRps } from '../../../../src/elements/inline/rp';
...
describe('elements/inline/rp', () => {
  ...
  it('creates a <rp> via document.createElement (never createElementNS)', () => {
    const el = createRp();
    const fake = el as unknown as FakeElement;

    expect(calls.createElement).toEqual(['rp']);
    expect(calls.createElementNS).toEqual([]);
    expect(fake.tagName).toBe('RP');
  });

  it('sets text via textContent (never HTML interpretation)', () => {
    const el = createRp('<b>(</b>');
    const fake = el as unknown as FakeElement;

    expect(fake.textContent).toBe('<b>(</b>');
  ...
  it('forwards curated global attributes', () => {
    const el = createRp('(', {
      id: 'open',
      className: 'ruby-fallback',
      title: 'Fallback open paren',
      lang: 'en',
      dir: 'ltr',
      role: 'note',
      hidden: true,
    ...
    expect(fake.id).toBe('open');
    expect(fake.className).toBe('ruby-fallback');
    expect(fake.title).toBe('Fallback open paren');
  ...
  it('supports structured ARIA input (mapped to aria-* attributes)', () => {
    const el = createRp(undefined, {
      aria: {
        label: 'Ruby fallback open',
        labelledby: 'ruby-1',
        hidden: true,
      },
    });
    ...
    expect(fake.getAttribute('aria-label')).toBe('Ruby fallback open');
    expect(fake.getAttribute('aria-labelledby')).toBe('ruby-1');
  ...
  it('enhanceRps is a no-op (but stable hook)', () => {
    expect(() => enhanceRps()).not.toThrow();
  });
});
