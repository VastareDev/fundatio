...
import { SAMP_TAG, createSamp, enhanceSamps } from '../../../../src/elements/inline/samp';
...
describe('elements/inline/samp', () => {
  it('creates a <samp> using document.createElement (never createElementNS)', () => {
    const env = installFakeDocument();

    const el = createSamp('Everything up-to-date') as unknown as FakeElement;

    expect(env.calls.createElement).toEqual([SAMP_TAG]);
    expect(env.calls.createElementNS).toEqual([]);
    expect(el.tagName).toBe('SAMP');
    expect(el.textContent).toBe('Everything up-to-date');
  ...
  it('applies GlobalAttrs (id/class/title/lang/dir/tabIndex/hidden) and mapped ARIA', () => {
    ...
    const el = createSamp('OK', {
      ...
      tabIndex: 0,
      hidden: false,
      aria: { label: 'Program output', labelledby: 'lbl', hidden: true },
    }) as unknown as FakeElement;

    expect(el.id).toBe('s1');
    expect(el.className).toBe('terminal');
    expect(el.title).toBe('Program output');
    expect(el.lang).toBe('en');
    expect(el.dir).toBe('ltr');
    expect(el.tabIndex).toBe(0);
    expect(el.hidden).toBe(false);

    expect(el.getAttribute('aria-label')).toBe('Program output');
  ...
  it('enhanceSamps is a stable no-op', () => {
    const env = installFakeDocument();

    expect(() => enhanceSamps()).not.toThrow();
