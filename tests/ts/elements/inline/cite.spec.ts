...
import { CITE_TAG, createCite, enhanceCites } from '../../../../src/elements/inline/cite';
...
describe('elements/inline/cite', () => {
  it('creates a <cite> using document.createElement (not createElementNS)', () => {
    const doc = installFakeDocument();

    try {
      const el = createCite();

      expect(doc.created).toEqual([CITE_TAG]);
      expect(doc.createdNS).toEqual([]);

      expect((el as unknown as FakeElement).tagName).toBe(CITE_TAG);
    } finally {
      doc.restore();
    }
  });
  ...
  it('assigns text via textContent', () => {
    ...
    try {
      const el = createCite('The Scream');

      expect(doc.created).toEqual([CITE_TAG]);
      expect((el as unknown as FakeElement).textContent).toBe('The Scream');
    } finally {
      doc.restore();
    }
  });
  ...
  it('applies global attributes (id, className, lang, dir, title, booleans)', () => {
    ...
    try {
      ...
      expect(fe.id).toBe('cite-1');
      expect(fe.className).toBe('work');
      expect(fe.lang).toBe('en');
      expect(fe.dir).toBe('ltr');
      expect(fe.title).toBe('Work title');
  ...
  it('maps structured ARIA into aria-* attributes', () => {
    ...
    try {
      const el = createCite(undefined, {
        aria: {
          label: 'Cited work title',
      ...
      const fe = el as unknown as FakeElement;
      expect(fe.getAttribute('aria-label')).toBe('Cited work title');
  ...
  it('enhancement hook is stable (no-op in v1.0.0)', () => {
    ...
    try {
      expect(() => enhanceCites()).not.toThrow();
    } finally {
      doc.restore();
    }
  });
