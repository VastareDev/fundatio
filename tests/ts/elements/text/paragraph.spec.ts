...
import { createP, enhancePs } from '../../../../src/elements/text/paragraph';
...
describe('elements/text/p', () => {
  ...
  it('creates a <p> via document.createElement (never createElementNS)', () => {
    const el = createP();
  ...
  it('forwards curated global attributes', () => {
    ...
    expect(fake.id).toBe('intro');
    expect(fake.className).toBe('lead');
  ...
  it('supports structured ARIA input (mapped to aria-* attributes)', () => {
    ...
    expect(fake.getAttribute('aria-label')).toBe('Intro paragraph');
  ...
  it('enhancePs is a no-op (but stable hook)', () => {
    expect(() => enhancePs()).not.toThrow();
  });
});
