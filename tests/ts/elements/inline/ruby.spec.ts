...
describe('elements/inline/ruby', () => {
  it('exposes tag and selector constants', () => {
    expect(RUBY_TAG).toBe('ruby');
    expect(RUBY_SELECTOR).toBe('ruby');
  });

  it('creates a <ruby> via document.createElement', () => {
    const doc = installFakeDocument();

    const el = createRuby();

    expect(doc.createElement).toHaveBeenCalledTimes(1);
    expect(doc.createElement).toHaveBeenCalledWith('ruby');
  ...
  it('applies global attributes', () => {
    ...
    expect(el.id).toBe('ruby1');
    expect(el.className).toBe('ruby');
    expect(el.title).toBe('Ruby annotation');
  ...
  it('maps structured ARIA into aria-* attributes', () => {
    ...
    const el = createRuby('base', {
      aria: { label: 'Ruby', labelledby: 'ruby-label', hidden: true },
    });

    expect(el.getAttribute('aria-label')).toBe('Ruby');
    expect(el.getAttribute('aria-labelledby')).toBe('ruby-label');
  ...
  it('enhancement hook is a stable no-op', () => {
    ...
    expect(() => enhanceRubies()).not.toThrow();
    expect(() => enhanceRubies(document as unknown as ParentNode)).not.toThrow();
  });
});
